...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.transeformers.update", [$transeformer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="feeder_id">{{ trans('cruds.transeformer.fields.feeder') }}</label>
                <select class="form-control select2 {{ $errors->has('feeder') ? 'is-invalid' : '' }}" name="feeder_id" id="feeder_id">
                    @foreach($feeders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('feeder_id') ? old('feeder_id') : $transeformer->feeder->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('boxes') ? 'is-invalid' : '' }}" name="boxes[]" id="boxes" multiple>
                    @foreach($boxes as $id => $box)
                        <option value="{{ $id }}" {{ (in_array($id, old('boxes', [])) || $transeformer->boxes->contains($id)) ? 'selected' : '' }}>{{ $box }}</option>
                    @endforeach
                </select>
                @if($errors->has('boxes'))
